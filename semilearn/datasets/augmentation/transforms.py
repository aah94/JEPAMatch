...
class MultiCropAugmentation:
    """
    Generates `num_crops` independently-augmented views of an image, used by
    JEPAMatch (semilearn/algorithms/jepamatch) for its global and local views
    (see paper Sec. 4: Multi-view Augmentation Strategy). Dataset-agnostic:
    parametrize `size`/`scale`/`mean`/`std` per dataset (e.g. crop scale
    (0.5, 1.0) for CIFAR-100 global views vs. (0.1, 0.4) for STL-10/Tiny-ImageNet
    local views).
    """

    def __init__(self, num_crops=2, size=32, scale=(0.5, 1.0), mean=None, std=None):
        self.size = size
        ...
        self.view_transform = transforms.Compose([
            transforms.RandomResizedCrop(self.size, scale=self.scale, interpolation=Image.BICUBIC),
    ...
    def __call__(self, img):
        return [self.view_transform(img) for _ in range(self.num_crops)]