...
import math
from semilearn.datasets.augmentation import RandAugment, RandomResizedCropAndInterpolation, str_to_interp_mode, MultiCropAugmentation
...
def get_imagenet(args, alg, name, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    if alg in JEPA_ALGORITHMS:
        scale_l = tuple(getattr(args, 'scale_l', (0.1, 0.4)))
        transform_local = MultiCropAugmentation(num_crops=args.num_local, size=args.local_size, scale=scale_l, mean=mean['imagenet'], std=std['imagenet'])
    ...
    ulb_dset = ImagenetDataset(root=os.path.join(data_dir, "train"), transform=transform_weak, alg=alg, ulb=True, medium_transform=transform_medium, strong_transform=transform_strong, include_lb_to_ulb=include_lb_to_ulb, lb_index=lb_dset.lb_idx, local_transform=transform_local)
...
class ImagenetDataset(BasicDataset, ImageFolder):
    def __init__(self, root, transform, ulb, alg, medium_transform=None, strong_transform=None, percentage=-1,
                 include_lb_to_ulb=True, lb_index=None, local_transform=None):
        self.alg = alg
        self.is_ulb = ulb
