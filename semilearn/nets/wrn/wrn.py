...
from semilearn.nets.jepa_modules import JEPAProjectionMixin
...
class WideResNet(nn.Module, JEPAProjectionMixin):
    def __init__(self, first_stride, num_classes, depth=28, widen_factor=2, drop_rate=0.0,
                 proj_out_dim=128, proj_hidden_dim=512, **kwargs):
        ...
        # JEPAMatch: projection head + (Adaptive)SIGReg, see semilearn/nets/jepa_modules.py.
        # Cheap relative to the backbone (3-layer MLP), so it is always built; algorithms
        # that don't use it (e.g. plain FlexMatch) simply never read outputs['proj'].
        self._build_jepa_projection(in_dim=channels[3], num_classes=num_classes,
                                     proj_hidden_dim=proj_hidden_dim, proj_out_dim=proj_out_dim)
    ...
    def forward(self, x, only_fc=False, only_feat=False, **kwargs):
        ...
        result_dict = {'logits': output, 'feat': out, 'proj': proj}
        return result_dict
